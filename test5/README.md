...
|    字段    |       类型       |      主键，外键     |是否为空|默认值|约束|说明|
|:-------:|:-------------:|:----------:|:-----------:|:-----------:|:-----------:|:-----------:|
|  id  |     int    |   主键   |否||AUTO_INCREMENT|借书记录ID|
|  borrowtime  |     date    |      |否|||借书时间|
|  bringbacktime  |     date    |      |是|空||还书时间,允许为空，如果为空则表示尚未归还|
|  admin_id  |     int    |    外键  |否|||处理此借书记录的管理员职工号|
|  reader_borrowid  |     int    |   外键   |否|||借书卡号|
|  source_id  |     int    |   外键   |否|||资源流水号，每一本图书的唯一编号|
